...
if (req.user.role != strings.adminRole && req.user.role != strings.hotelAdminRole) return res.status(500).send({mensaje: strings.permissionsError}); 
  ...
  exports.getEventTypes = (req, res) => {
    if (req.user.role != strings.adminRole) return res.status(500).send({ message: strings.permissionsError });
  ...
  exports.getEventTypeById = (req, res) => {
    var eventTypeId = req.params.eventTypeID;
    if (req.user.role != strings.adminRole) return res.status(500).send({ message: strings.permissionsError });
    EventType.findById(eventTypeId, (err, eventType) => {
      if (err) {
        res.status(500).send({ message: strings.requestError });
      } else if (eventType) {
        res.send({ eventType });
